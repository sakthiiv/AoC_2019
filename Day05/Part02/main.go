...
func jumpIfTrue(mode1, mode2, idx int, data []int) int {
	x, y := getParams(mode1, data, idx+1), getParams(mode2, data, idx+2)
	if x != 0 {
		return y
	} else {
...
func jumpIfFalse(mode1, mode2, idx int, data []int) int {
	x, y := getParams(mode1, data, idx+1), getParams(mode2, data, idx+2)
	if x == 0 {
		return y
	} else {
...
func lessThen(mode1, mode2, idx int, data []int) {
	x, y := getParams(mode1, data, idx+1), getParams(mode2, data, idx+2)
	if x < y {
		data[data[idx+3]] = 1
	} else {
...
func equals(mode1, mode2, idx int, data []int) {
	x, y := getParams(mode1, data, idx+1), getParams(mode2, data, idx+2)
	if x == y {
		data[data[idx+3]] = 1
	} else {
...
func PerformDiagnosis(data []int, input int) []int {
	...
	for data[idx] != 99 {
		...
		case 4:
			...
			idx += 2
		case 5:
			idx = jumpIfTrue(mode1, mode2, idx, data)
		case 6:
			idx = jumpIfFalse(mode1, mode2, idx, data)
		case 7:
			lessThen(mode1, mode2, idx, data)
			idx += 4
		case 8:
...
func main() {
	...
	PerformDiagnosis(instructions, 5)
}
